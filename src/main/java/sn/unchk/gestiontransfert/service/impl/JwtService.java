...
public class JwtService {
    ...
    public UtilisateurModel getUserFromToken(String token) {
        ...
        byte[] photo = claims.get("photo", byte[].class);
        String numeroPiece = claims.get("numeroPiece", String.class);
        byte[] photoPiece = claims.get("photoPiece", byte[].class);

        // Pour solde, on convertit depuis Number si nécessaire
        BigDecimal solde = null;
        Object soldeObj = claims.get("solde");
        if (soldeObj instanceof Number) {
            solde = BigDecimal.valueOf(((Number) soldeObj).doubleValue());
        }

        String qrcode = claims.get("qrcode", String.class);
        Statut statut = Statut.valueOf(claims.get("statut", String.class));
        ...
        user.setEmail(email);
        user.setPhoto(photo);
        user.setNumeroPiece(numeroPiece);
        user.setPhotoPiece(photoPiece);
        user.setSolde(solde);
